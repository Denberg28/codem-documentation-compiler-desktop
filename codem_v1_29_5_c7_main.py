...
def protect_nested_git_folder(project_dir: Path) -> tuple[bool, str]:
    ...
    CodeM v1.29.5-c7-c2 publishes the project folder from its parent repo, so a nested
...
def normalize_markdown_lists(text: str) -> str:
    """
    GitHub-friendly Markdown normalization without destroying indentation.

    Markdown leading spaces are meaningful. Nested bullets, nested checklists,
    numbered substeps, and indented text must survive Save Docs / Save All.
    """
    list_sections = {
        "notes",
        "evidence",
    ...
    for raw_line in text.splitlines():
        ...
        # Fix common toolbar/normalizer artifacts while preserving the original indent.
        ...
        # If the user intentionally indented the line, preserve it as written.
...
class CodeMApp(tk.Tk):
    ...
    def _active_docs_editor(self) -> tk.Text | None:
        """
        Return the editor widget for the currently selected Docs tab.
        """
        try:
            selected_tab = self.docs_notebook.select()
            selected_label = self.docs_notebook.tab(selected_tab, "text")
        except tk.TclError:
            return None

        editor = self.editor_widgets.get(str(selected_label))
    ...
    def _insert_hidden_text_block(self) -> None:
        """
        Insert a GitHub-compatible collapsible Markdown block.

        This uses HTML <details>/<summary>, which GitHub Markdown supports.
        ...
        try:
            ...
            editor.focus_set()
            self.status_var.set("Hidden text block inserted.")
        except tk.TclError:
            pass
