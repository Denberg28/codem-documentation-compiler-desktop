...
def protect_nested_git_folder(project_dir: Path) -> tuple[bool, str]:
    ...
    CodeM v1.29.5 publishes the project folder from its parent repo, so a nested
...
def normalize_markdown_lists(text: str) -> str:
    """
    GitHub-friendly Markdown normalization without creating false entries.

    Rules:
    - Removes empty placeholder bullets.
    - Preserves valid headings, bullets, checklists, numbered lists, tables, links, quotes, and code blocks.
    - Converts plain lines under list-style sections into bullets.
    - Avoids creating '- [ ] - item' or '- - item' artifacts.
    """
    list_sections = {
        "notes",
        "evidence",
    ...
    for raw_line in text.splitlines():
        ...
        # Fix common toolbar/normalizer artifacts.
        ...
        already_structured = (
            ...
            or bool(re.match(r"^-\s+\[[ xX]\]\s+", stripped))
